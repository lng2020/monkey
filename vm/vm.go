...
func (vm *VM) pushClosure(constIndex int, numFree int) error {
	constants := vm.constants[constIndex]
	function, ok := constants.(*object.CompiledFunction)
	if !ok {
		return fmt.Errorf("not a function: %+v", constants)
	}
...
func (vm *VM) executeBinaryStringOperation(
	op code.Opcode,
	left, right object.Object) error {
	if op != code.OpAdd {
...
func (vm *VM) executeIndexExpression(left, index object.Object) error {
	...
	default:
		return fmt.Errorf("inedx operator not supported: %s", left.Type())
	}
}
